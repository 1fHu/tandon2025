...
Node* reverse(const Node* hp){
    ...
    while (hp != nullptr){
       newHead = new Node{hp->data, newHead};
       hp = hp->next;
    }
    return newHead;
}
...
const Node* isSublist(const Node* pattern, const Node* target){
    ...
    while (target != nullptr && pattern != nullptr){
        ...
        while (tar != nullptr && pat != nullptr && tar->data == pat->data){
            tar = tar->next;
            pat = pat->next;

        }
        if (pat == nullptr){ // all number in pattern matches target
            return target;
        }
...
const Node* sharedListBruteForce(const Node* listA, const Node* listB){
    const Node* aPtr = listA; 

    while (aPtr != nullptr){
        const Node* bPtr = listB;
        while (bPtr != nullptr){
            if (aPtr == bPtr) return bPtr;
            bPtr = bPtr->next;
        }
        aPtr = aPtr->next;
